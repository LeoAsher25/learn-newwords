...
interface WordInputTableProps {
  ...
  onAddExample: (wordIndex: number) => void;
  onRemoveExample: (wordIndex: number, exampleIndex: number) => void;
      ...
      <table className="w-full table-fixed text-left text-sm">
        ...
        <tbody>
          {rowKeys.map((rowKey, index) => {
            ...
            const examples = word.examples?.length > 0 ? word.examples : [""];
            ...
            return (
              <Suspense key={rowKey}>
                ...
                <tr key={rowKey + "-examples-row"} className="align-top">
                  <td></td>
                  <td colSpan={2} className="px-1 md:px-3 py-2 align-middle">
                    <div className="space-y-2">
                      {examples.map((example, exampleIndex) => (
                        <div
                          key={`${rowKey}-example-${exampleIndex}`}
                          ...
                          <input
                            ...
                            className="w-full rounded-lg border border-slate-300 px-3 py-1 h-10 text-sm outline-none transition focus:ring-2 focus:ring-slate-200"
                            placeholder="Ví dụ: I learn English every day."
                          />
                          <button
                            type="button"
                            onClick={() => onRemoveExample(index, exampleIndex)}
                            className="inline-flex size-8 shrink-0 items-center justify-center rounded-md border border-rose-500/35 text-sm text-rose-500/85 hover:bg-rose-500/12"
                            aria-label={`Xóa ví dụ ${exampleIndex + 1} của từ số ${index + 1}`}>
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 24 24"
                              fill="none"
